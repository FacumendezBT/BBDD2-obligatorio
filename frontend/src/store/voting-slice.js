...
export const enviarVotoCompleto = createAsyncThunk('voting/enviarVotoCompleto', async (votoData, { rejectWithValue }) => {
  try {
    ...
    prepararVotoCompleto: (state, action) => {
      ...
      state.votes.forEach((vote) => {
        ...
        if (vote.lista_seleccionada.tipo === 'Blanco') {
          // Para votos en blanco, no agregamos papeletas, solo marcamos la elección como votada
          return;
        }
      ...
      state.votoCompleto = {
        ...
        votos_blancos: state.votes.filter((vote) => vote.lista_seleccionada.tipo === 'Blanco').map((vote) => vote.eleccion_id),
      };
    },
  },
...
export const selectVoteForElection = (state, eleccionId) => state.voting.votes.find((vote) => vote.eleccion_id === eleccionId);
